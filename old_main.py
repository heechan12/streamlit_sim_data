...
if __name__ == '__main__':
    ...
    if st.button('Code 확인', type="primary"):
        if not isCorrectLength(input_old_code, CODE_LENGTH) or not isHex(input_old_code):
            if not isCorrectLength(input_old_code, CODE_LENGTH):
                st.warning(f'''코드 길이가 :red[{CODE_LENGTH}자]가 아닙니다. (:red[{len(input_old_code)} 자])''', icon="⚠️")
            if not isHex(input_old_code):
    ...
    if st.session_state.NEW_SIM_DATA is not None:
        ...
        if st.button('업데이트 정보 입력', type="primary"):
            ...
            # <--- 여기서부터 함수로 작성 필요
            temp_list_code_0 = list(st.session_state.NEW_SIM_DATA.code0)
            temp_list_code_1 = list(st.session_state.NEW_SIM_DATA.code1)

            for i in range(3):
                temp_list_code_0[i] = mcc[i]
                temp_list_code_1[i] = mnc[i]

            st.session_state.NEW_SIM_DATA.code0 = ''.join(temp_list_code_0)
            st.session_state.NEW_SIM_DATA.code1 = ''.join(temp_list_code_1)
            ...
            # 정상동작 확인 용도
            st.write("OLD Data : " + st.session_state.OLD_SIM_DATA.code0)
            st.write("NEW Data : " + st.session_state.NEW_SIM_DATA.code0)
            st.write("OLD Data : " + st.session_state.OLD_SIM_DATA.code1)
            st.write("NEW Data : " + st.session_state.NEW_SIM_DATA.code1)
