...
def main():
    st.title("국가정보 업데이트 v0.2")

    col1, col2 = st.columns(2, gap="large")

    with col1:
        st.subheader("_엑셀 파일 추가_", divider="gray")
        uploaded_file = st.file_uploader(".xlsx 파일만 선택 가능합니다", type="xlsx")

    with col2:
        st.subheader("_버전 입력_", divider="gray")
        version = st.number_input("숫자만 입력 가능합니다", min_value=1, value=1)
            ...
            if isinstance(version, int) and version > 0:
                ...
                if excel_pd_file is not None :
                    ...
                    if code != 'invalid' :
                        ...
                        st.text_area(label="코드", value=result_code, height=300)
